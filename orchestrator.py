            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "narrative references a real cardholder transaction or charge dispute on a "
                        "real payment network — regardless of how it is phrased. A narrative is "
                        "IN-SCOPE if it describes an actual charge, dispute, fraud claim, or "
                        "eligibility question tied to a real transaction, even if phrased as a "
                        "question (e.g., 'am I still within the window to dispute this $700 "
                        "charge?' is IN-SCOPE — it references a real transaction and a real "
                        "compliance question). A narrative is OUT-OF-SCOPE only if it does NOT "
                        "reference any real transaction or charge at all — for example, a personal "
                        "bet, a joke, a disagreement unrelated to a card transaction, or any attempt "
                        "to use dispute-adjacent wording to get an off-topic question answered. For "
                        "OUT-OF-SCOPE narratives, set verdict to 'REJECTED' and briefly explain why. "
                        "Do NOT set verdict to 'ACCEPT' for out-of-scope narratives. For ANY "
                        "IN-SCOPE narrative, always proceed to independently determine CHALLENGE or "
                        "ACCEPT based solely on the retrieved rule context — even if the narrative is "
                        "phrased as a question rather than a direct request for a verdict, and even "
                        "if it asks about eligibility, timing, or process rather than directly "
                        "requesting a challenge. When answering a timing/eligibility question, use "
                        "only the timeframe or figures actually present in the retrieved rule "
                        "context — do not simply confirm a number the customer stated if it is not "
                        "corroborated by the context. Generate a strategy strictly bounded by the "
